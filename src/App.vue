...
const step         = ref(0)
const savedCorners = ref([])
const pieceId      = ref(null)
const piecePos     = ref(null)
const detectedNow  = ref([])
const lastDetected = ref(null)

// Controlli preprocessing
...
const STEPS = [
  ...
  { label: 'Pezzo',    desc: 'MARCATORE PEZZO',        short: 'PC' },
]
...
function preprocess(imageData) {
  ...
  for (let i = 0; i < data.length; i += 4) {
    ...
    const v    = Math.min(255, Math.max(0, gray * c + b))
...
function pointToGrid(px, py) {
  ...
  return { col: Math.floor(u*GRID_COLS)+1, row: Math.floor(v*GRID_ROWS)+1 }
}
...
function drawGrid(ctx) {
  ...
  ctx.save(); ctx.strokeStyle='rgba(100,180,255,0.35)'; ctx.lineWidth=1
...
function drawCellHighlight(ctx, col, row) {
  ...
  const p00=blerp(u0,v0),p10=blerp(u1,v0),p11=blerp(u1,v1),p01=blerp(u0,v1)
  ctx.beginPath()
...
function loop() {
  ...
  if (!video||!canvas||video.readyState<2) { rafId=requestAnimationFrame(loop); return }
  ...
  // Debug canvas
  const dbg=debugRef.value
  if (dbg) {
    if (dbg.width!==w||dbg.height!==h) { dbg.width=w; dbg.height=h }
    dbg.getContext('2d').putImageData(processed,0,0)
  }
  ...
  if (step.value<5) {
    markers.forEach(m => {
      const isSaved=cornerIds.has(m.id)
      drawMarkerBox(ctx, m,
        isSaved ? '#00ff88' : '#ffaa00',
        isSaved ? savedCorners.value.find(c=>c.id===m.id)?.short : `ID ${m.id}`
      )
    })
    const candidate=markers.find(m=>!cornerIds.has(m.id))
    if (candidate) {
      const center=markerCenter(candidate)
      lastDetected.value={ id:candidate.id, x:center.x, y:center.y }
    } else {
    ...
    savedCorners.value.forEach(sc=>{
      if (byId[sc.id]) drawMarkerBox(ctx,byId[sc.id],'#ffffff44',sc.short)
    })
  ...
  if (now-lastFps>=1000) { fps.value=frameCount; frameCount=0; lastFps=now }
...
function confirmStep() {
  ...
  savedCorners.value.push({
    ...
    short: STEPS[step.value].short,
    label: STEPS[step.value].label,
    desc:  STEPS[step.value].desc
  })
  lastDetected.value=null
  step.value++
}
...
function reset() {
  step.value=0; savedCorners.value=[]; pieceId.value=null
  piecePos.value=null; lastDetected.value=[]
}
  ...
  <div class="app">
    <header>
      ...
      <div class="badges">
        ...
        <button class="btn-sm" @click="toggleCamera">{{ useFront?'📷 Post.':'🤳 Front.' }}</button>
    ...
    <div class="panel slim">
      <div class="slider-row">
        <label>☀ Luminosità <b>{{ brightness }}</b></label>
        <input type="range" min="-50" max="150" v-model.number="brightness" />
      </div>
      <div class="slider-row">
        <label>◑ Contrasto <b>{{ contrast.toFixed(1) }}</b></label>
        <input type="range" min="0.5" max="4" step="0.1" v-model.number="contrast" />
      </div>
    </div>

    <!-- Debug canvas -->
    <div class="panel slim">
      <h2>🔬 Preview elaborata — Marker rilevati:
        <b style="color:#00ff88">{{ detectedNow.length }}</b>
        <span style="color:#888;font-weight:400"> ID: {{ detectedNow.join(', ')||'—' }}</span>
      </h2>
      <canvas ref="debugRef" style="width:100%;border-radius:6px;margin-top:.4rem" />
    </div>

    <!-- Step calibrazione angoli -->
    <div v-if="step<4" class="panel">
      ...
      <p class="hint">Mostra il marker da usare per <b>{{ STEPS[step].desc }}</b></p>
      ...
      <div class="detect-box" :class="lastDetected?'found':'waiting'">
        <template v-if="lastDetected">
          <span>✓ Rilevato <b>ID {{ lastDetected.id }}</b></span>
        ...
        <template v-else>
          ...
          <span style="color:#666">In attesa del marker...</span>
        </template>
      </div>
    </div>
    ...
    <div v-else-if="step===4" class="panel">
      <div class="step-header">
        ...
        <h2>Registra <span class="step-desc">MARCATORE PEZZO</span></h2>
      </div>
      ...
      <div class="detect-box" :class="lastDetected?'found':'waiting'">
        <template v-if="lastDetected">
          <span>✓ Rilevato <b>ID {{ lastDetected.id }}</b></span>
          <button class="btn-confirm" @click="confirmPiece">▶ INIZIA A GIOCARE</button>
        </template>
        <template v-else>
          <span class="spin">⧖</span>
          <span style="color:#666">In attesa del marker pezzo...</span>
        </template>
      </div>
    </div>
    ...
    <div v-else class="panel playing">
      ...
      <div class="piece-info" :class="piecePos?'active':'idle'">
        <template v-if="piecePos">
          ...
          <span class="piece-coords">Col <b>{{ piecePos.col }}</b> · Riga <b>{{ piecePos.row }}</b></span>
        </template>
        <template v-else>
...
.badge.fps { background:#1a1a2e; color:#7cb3ff; padding:3px 10px; border-radius:20px; font-size:.75rem; font-weight:700 }
.btn-sm { background:#1a1a2e; border:1px solid #333; border-radius:6px; color:#7cb3ff; padding:.35rem .7rem; font-size:.82rem; cursor:pointer }
.btn-sm.danger { color:#ff6666; border-color:#ff666633 }

.viewport { position:relative; width:100%; max-width:900px; aspect-ratio:16/9; background:#000; border:1px solid #222; border-radius:8px; overflow:hidden }
.hidden { display:none }
canvas { width:100%; height:100%; display:block }

.panel { width:100%; max-width:900px; background:#111; border:1px solid #222; border-radius:10px; padding:1rem; display:flex; flex-direction:column; gap:.6rem }
.panel.slim { padding:.7rem 1rem; gap:.4rem }
h2 { font-size:.9rem; color:#c8a84b }

.slider-row { display:flex; align-items:center; gap:.8rem }
.slider-row label { font-size:.8rem; color:#888; min-width:130px }
.slider-row input { flex:1; accent-color:#c8a84b }

.step-header { display:flex; align-items:center; gap:.7rem }
.step-pill { background:#c8a84b22; color:#c8a84b; border:1px solid #c8a84b44; border-radius:20px; padding:2px 10px; font-size:.8rem; font-weight:700; white-space:nowrap }
.step-desc { color:#ffaa00 }
.hint { font-size:.82rem; color:#777 }

.saved-list { display:flex; flex-direction:column; gap:.3rem }
.saved-chip { display:flex; align-items:center; gap:.6rem; background:#0d2b1a; border:1px solid #00ff8822; border-radius:6px; padding:.4rem .7rem }
.saved-chip.mini { padding:.25rem .5rem }
.saved-short { font-family:monospace; font-weight:900; color:#00ff88; min-width:28px }
.saved-desc { font-size:.8rem; color:#888; flex:1 }
.saved-id { font-family:monospace; color:#fff; font-size:.85rem }

.detect-box { border-radius:8px; padding:.8rem 1rem; display:flex; align-items:center; gap:.8rem; flex-wrap:wrap; border:1px solid }
...
.btn-confirm { background:#c8a84b; border:none; border-radius:8px; color:#000; font-weight:900; padding:.5rem 1rem; font-size:.9rem; cursor:pointer }
...
.piece-info { border-radius:8px; padding:.8rem 1rem; display:flex; align-items:center; gap:.8rem; flex-wrap:wrap; border:1px solid; font-size:.95rem }
.piece-info.active { background:#0d1f2b; border-color:#00aaff44 }
.piece-info.idle { background:#1a1a1a; border-color:#333 }
.piece-coords { margin-left:auto; font-family:monospace; font-size:1.1rem; font-weight:700; color:#ffaa00 }
